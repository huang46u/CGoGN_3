#ifndef CGOGN_MODULE_ALPHA_SAMPLES_SPHERE_DEBUGGER_H_
#define CGOGN_MODULE_ALPHA_SAMPLES_SPHERE_DEBUGGER_H_
...
class AlphaSamplesSphereDebugger : public ViewModule
{
public:
	template <typename T>
	...
	AlphaSamplesSphereDebugger(const App& app)
		: ViewModule(app, "AlphaSamplesSphereDebugger (" + std::string{mesh_traits<POINTS>::name} + ")"),
...
protected:
	...
	void key_press_event(View* view, int32 key_code) override
	{
		...
		if (get_source_basic(p, src))
		{
			...
			if (!needs_cluster && (p.cluster_source_mesh_ != src.mesh || p.cluster_spheres_mesh_ != p.spheres_mesh_))
				needs_cluster = true;
...
private:
	...
	void init_parameters(POINTS& m, Parameters& p)
	{
		...
		const std::string debug_cluster_name = base_name + "_samples_debug";
		...
		const std::string debug_sphere_name = base_name + "_samples_debug_sphere";
	...
	bool get_source_basic(Parameters& p, SourceBasic& out)
	{
		...
		if (!out.mesh || !out.cluster_position)
			return false;
		return true;
	}
	...
	bool get_source_data(Parameters& p, SourceData& out)
	{
		...
		if (!out.mesh || !out.position || !out.cluster_position || !out.normal || !out.area || !out.knn ||
			!out.quadric || !out.line_quadric)
			return false;
		return true;
	}
...
#endif // CGOGN_MODULE_ALPHA_SAMPLES_SPHERE_DEBUGGER_H_
