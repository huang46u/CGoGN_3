...
private:
	struct Parameters
	{
		...
		POINTS* cluster_source_mesh_ = nullptr;
		POINTS* cluster_spheres_mesh_ = nullptr;
...
protected:
	...
	void left_panel() override
	{
		...
		const bool has_cluster_attr = (p.spheres_cluster_ != nullptr);
		...
		if (!has_cluster_attr)
		{
			ImGui::TextColored(ImVec4(1, 1, 0, 1),
							   "Sphere cluster attribute not found. Run Compute clusters in training.");
			return;
		}
		...
		ImGui::Separator();
		ImGui::Text("Source: Alpha Inside (Projected)");
		if (ImGui::Button("Refresh Cluster Cache"))
		{
			refresh_cluster_cache(p);
			update_debug_meshes(p, p.picked_sphere_);
		}
		...
		ImGui::Separator();
		ImGui::Text("Update All Spheres");
		if (ImGui::Button("All: SQEM (one-step)"))
		{
			if (src_ready && p.spheres_mesh_)
			{
				refresh_cluster_cache(p);
				foreach_cell(*p.spheres_mesh_, [&](PVertex v) -> bool {
		...
		if (ImGui::Button("All: SQEM + Euclidean (iter)"))
		{
			if (src_ready && p.spheres_mesh_)
			{
				refresh_cluster_cache(p);
				foreach_cell(*p.spheres_mesh_, [&](PVertex v) -> bool {
					update_sphere_sqem_euclidean(p, v);
					return true;
				});
			}
		}
		if (ImGui::Button("All: SQEM + Line Quadric (fixed alpha)"))
		{
			if (src_ready && p.spheres_mesh_)
			{
				refresh_cluster_cache(p);
		...
		if (ImGui::Button("All: SQEM + Line Quadric (free radius)"))
		{
			if (src_ready && p.spheres_mesh_)
			{
				refresh_cluster_cache(p);
				foreach_cell(*p.spheres_mesh_, [&](PVertex v) -> bool {
					update_sphere_sqem_line_quadric_free_radius(p, v);
					return true;
				});
			}
		}
...
private:
	...
	void init_parameters(POINTS& m, Parameters& p)
	{
		...
		if (ends_with(selected_name, "_alpha_inside"))
		{
			base_name = selected_name.substr(0, selected_name.size() - std::string("_alpha_inside").size());
		}
		else if (ends_with(selected_name, "_spheres"))
		{
			base_name = selected_name.substr(0, selected_name.size() - std::string("_spheres").size());
		}
		...
		if (p.spheres_mesh_)
		{
			...
			p.spheres_color_ = get_attribute<Vec4, PVertex>(*p.spheres_mesh_, "color");
			p.spheres_cluster_color_ = get_or_add_attribute<Vec4, PVertex>(*p.spheres_mesh_, "cluster_color");
			p.spheres_cluster_ = get_attribute<std::vector<PVertex>, PVertex>(*p.spheres_mesh_, "cluster");
		}
	...
	void setup_debug_render(Parameters& p)
	{
		...
		if (p.spheres_mesh_ && p.spheres_position_ && p.spheres_radius_ && (p.spheres_color_ || p.spheres_cluster_color_))
		{
			...
			auto color_attr = p.spheres_color_ ? p.spheres_color_ : p.spheres_cluster_color_;
			pcr_->set_vertex_color(*selected_view_, *p.spheres_mesh_, color_attr);
		...
		{
			pcr_->set_vertex_position(*selected_view_, *p.debug_cluster_mesh_, p.debug_cluster_position_);
	...
	void filter_cluster_components(const SourceData& src, const std::vector<PVertex>& cluster,
								   uint32& kept_components, uint32& total_components,
								   std::vector<uint32>& kept_sizes,
								   std::vector<PVertex>* filtered = nullptr)
	{
		kept_components = 0;
		total_components = 0;
		kept_sizes.clear();
		...
		for (PVertex v : cluster)
		{
			...
			if (component.size() > 3)
			{
				...
				if (filtered)
					filtered->insert(filtered->end(), component.begin(), component.end());
			}
		}
	}

	bool refresh_cluster_cache(Parameters& p)
	{
		SourceBasic src;
		if (!get_source_basic(p, src))
			return false;
		if (!p.spheres_mesh_ || !p.spheres_position_ || !p.spheres_radius_ || !p.spheres_cluster_)
			return false;

		const uint32 nb_spheres = nb_cells<PVertex>(*p.spheres_mesh_);
		if (p.spheres_position_->size() < nb_spheres || p.spheres_radius_->size() < nb_spheres)
			return false;
		if (p.spheres_cluster_->size() < nb_spheres)
			return false;
		const uint32 cluster_size = nb_spheres;
		...
		for (uint32 i = 0; i < nb_spheres; ++i)
			p.clusters_[i] = (*p.spheres_cluster_)[i];

		SourceData src_full;
		const bool has_src_full = get_source_data(p, src_full);
		...
		if (has_src_full)
		{
			for (uint32 i = 0; i < cluster_size; ++i)
			{
				uint32 kept = 0;
				uint32 total = 0;
				filter_cluster_components(src_full, p.clusters_[i], kept, total, p.cluster_component_sizes_[i]);
		...
		p.cluster_source_mesh_ = src.mesh;
		p.cluster_spheres_mesh_ = p.spheres_mesh_;
		return true;
	}
